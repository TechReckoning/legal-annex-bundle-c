...
import { Download, ArrowUp, ArrowDown, Trash, FolderPlus } from '@phosphor-icons/react';
...
function App(): React.ReactElement {
  ...
  const safeOpisFormatting = React.useMemo(() => {
    try {
      ...
      const formatting = { ...defaultFormattingOptions, ...opisFormatting };
      // Clear invalid logoFile (File objects can't be stored in localStorage)
      if (formatting.logoFile && !(formatting.logoFile instanceof File)) {
        formatting.logoFile = undefined;
      }
      return formatting;
    } catch (error) {
  ...
  const safeCoverFormatting = React.useMemo(() => {
    try {
      ...
      const formatting = { ...defaultCoverFormattingOptions, ...coverFormatting };
      // Clear invalid logoFile (File objects can't be stored in localStorage)
      if (formatting.logoFile && !(formatting.logoFile instanceof File)) {
        formatting.logoFile = undefined;
      }
              ...
              <Tabs defaultValue="preview" className="w-full">
                ...
                <TabsContent value="preview" className="space-y-4">
                  <PreviewPanel
                    ...
                    opisFormatting={safeOpisFormatting}
                    coverFormatting={safeCoverFormatting}
                    onAnnexSelect={(annex) => setSelectedAnnexId(annex.id)}
                  />
