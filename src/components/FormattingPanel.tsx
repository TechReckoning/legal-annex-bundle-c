...
const opisAlignmentOptions = [
  { value: 'left', label: 'Stânga' },
];

const coverAlignmentOptions = [
...
export const FormattingPanel: React.FC<FormattingPanelProps> = ({
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Formatare Listă de Anexe</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Inter - font open-source optimizat pentru PDF
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <SelectContent>
                  {opisAlignmentOptions.map((option) => (
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="opis-add-stamp"
                checked={opisFormatting.addStamp || false}
                onCheckedChange={(checked) => updateOpisFormatting({ addStamp: !!checked })}
              />
              <Label htmlFor="opis-add-stamp">Adaugă ștampilă pe paginile de conținut</Label>
            </div>

            {opisFormatting.addStamp && (
              <div className="grid grid-cols-2 gap-4 pl-6">
                <div>
                  <Label htmlFor="opis-stamp-text">Text ștampilă</Label>
                  <Input
                    id="opis-stamp-text"
                    value={opisFormatting.stampText || 'Conform cu originalul'}
                    onChange={(e) => updateOpisFormatting({ stampText: e.target.value })}
                    placeholder="Conform cu originalul"
                  />
                </div>

                <div>
                  <Label htmlFor="opis-stamp-position">Poziție ștampilă</Label>
                  <Select 
                    value={opisFormatting.stampPosition || 'bottom-right'} 
                    onValueChange={(value: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right') => 
                      updateOpisFormatting({ stampPosition: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  <Label htmlFor="opis-stamp-font-size">Dimensiune font ștampilă</Label>
                  <Input
                    id="opis-stamp-font-size"
                    type="number"
                    min="6"
                    max="24"
                    value={opisFormatting.stampFontSize || 10}
                    onChange={(e) => updateOpisFormatting({ stampFontSize: Number(e.target.value) })}
                  />
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {coverAlignmentOptions.map((option) => (
                    <SelectItem key={option.value} value={option.value}>
                      {option.label}
                    </SelectItem>
                  ))}
